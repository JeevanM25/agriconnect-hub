...
import { MapPin, Package, IndianRupee, Calendar, User, Phone, Search } from 'lucide-react';
...
export function CropSearch({ crops }: CropSearchProps) {
  ...
  const handleRequestMeeting = (crop: Crop) => {
    ...
    toast.success(`Meeting request sent to ${crop.farmerName}!`);
    setMeetingDate('');
    setMeetingTime('');
  };
        ...
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <Input
                placeholder="Location (e.g., Shivamogga)"
                value={location}
      ...
      {hasSearched && (
        <div className="space-y-4">
          <p className="text-muted-foreground">
            Found {filteredCrops.length} result{filteredCrops.length !== 1 ? 's' : ''}
          </p>
                    ...
                    <div className="flex-1 p-4">
                      ...
                      <Dialog>
                        <DialogTrigger asChild>
                          <Button className="w-full sm:w-auto bg-middleman hover:bg-middleman/90">
                        ...
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>Schedule Meeting with {crop.farmerName}</DialogTitle>
                          </DialogHeader>
                          <div className="space-y-4 py-4">
                            ...
                            <Button
                              onClick={() => handleRequestMeeting(crop)}
                              className="w-full bg-middleman hover:bg-middleman/90"
                            >
                              Send Request
                            </Button>
                          </div>
